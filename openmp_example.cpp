...
public:
    ...
    inline float4(float a, float b, float c) :  mmvalue(_mm_setr_ps(0, a,b,c)) {}
    inline float4(float a, float b, float c, float d) :  mmvalue(_mm_setr_ps(a,b,c,d)) {}
    inline float4(__m128 m) : mmvalue(m) {}

    inline float4 operator+(const float4& b) const { return _mm_add_ps(mmvalue, b.mmvalue);}
    inline float4 operator-(const float4& b) const { return _mm_sub_ps(mmvalue, b.mmvalue);}
    inline float4 operator*(const float4& b) const { return _mm_mul_ps(mmvalue, b.mmvalue);}
    inline float4 operator/(const float4& b) const { return _mm_div_ps(mmvalue, b.mmvalue);}
    inline float4 operator-()               const { return _mm_xor_ps(mmvalue, SIGNMASK); }
    ...
    inline float4 operator+(float b) const { return _mm_add_ps(mmvalue, _mm_set1_ps(b)); }
    inline float4 operator-(float b) const { return _mm_sub_ps(mmvalue, _mm_set1_ps(b)); }
    inline float4 operator*(float b) const { return _mm_mul_ps(mmvalue, _mm_set1_ps(b)); }
    inline float4 operator/(float b) const { return _mm_div_ps(mmvalue, _mm_set1_ps(b)); }
